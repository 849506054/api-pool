...
- [ ] **复核错误分类与端点处置矩阵** — 建立 HTTP 4xx/429、余额不足、配额耗尽、502/503/504、连接错误、非流式超时、单请求饿死、假成功和流式事务失败的“错误类型 → 重试 → 冷却 → 探活 → 切换”矩阵；确认请求级异常不会被错误升级为端点级故障，只有明确端点故障才进入冷却。
...
- [ ] **观察本轮三个修复的实际行为** — 仅基于真实业务样本复核缓存保护/延迟回切、并发探活误判和流式事务日志；精确关联同一请求的 API Pool 日志、探活结果和真实请求结果，不主动向生产端点发送探测请求，不因单个偶发事件直接调整参数。
- [ ] **复核故障转移控制路径** — 在真实问题证明必要时，检查并发探活两阶段等待、恢复探活与真实请求竞态、终极兜底锁定与缓存保护/手动覆盖的组合行为；优先删除不可靠机制或修复明确条件，不引入新的复杂路由状态机、ModelRoute 或独立 Upstream 建模。
- [ ] **按真实故障触发复核对话日志管线** — 仅当日志写入明确造成请求阻塞、SQLite 锁冲突、后台线程积压、存储异常或妨碍故障归因时评估；没有真实证据时不改队列、事务结构、日志内容或 30 天保留策略。
