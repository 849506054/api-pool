...
- [ ] **复核故障转移控制路径** — 在真实问题证明必要时，检查并发探活两阶段等待、恢复探活与真实请求竞态、终极兜底锁定与缓存保护/手动覆盖的组合行为；优先删除不可靠机制或修复明确条件，不引入新的复杂路由状态机、ModelRoute 或独立 Upstream 建模。（组合行为矩阵已纳入下方 P2 整合评审 §6，其「不加复杂路由状态机」为本工作流的硬边界）
