...
## 2026-09-20 敏感词过滤新增「零宽插入」规则类型 已部署生产
**需求（用户）**：在现有替换方式之外，加入零宽替代作为可选。
**实现**：`ContentFilter` 新增规则类型 `zero_width` —— 保留命中文本、只在片段内部插入不可见字符（默认 U+200B，`char` 可覆盖、`at` 指定位置或默认中点、`regex: true` 可让 pattern 走正则）。命中时替换值是保留原文的可调用对象，`_repl` 按 `callable` 分派；替换型规则行为不变。管理 API/UI 同步：类型下拉新增「零宽插入」（只填 pattern），规则徽章显示「🟢零宽 / → 插入零宽字符（词形不变）」，`/api/content-filter/test` 预览把 U+200B/200C/2060/00AD 渲染成 `⟨ZW⟩/⟨ZWNJ⟩/⟨WJ⟩/⟨SHY⟩`，避免不可见字符在预览里看不出效果。
**配置**：生产 `content_filter.json` 经 `PUT /api/content-filter` 字段级追加一条 `{"type":"zero_width","pattern":"recal​l_fact"}`，`dictionary_version` → `2026-09-20-zw`，其余三条替换型规则与 targets 原样保留（热重载，无重启）。配置文件本体不含不可见字符（零宽由规则在运行时插入）。
