...
- [ ] **[P0] 建立请求分段计时基线（DEBUG 可选）** — 已接入现有 `API_POOL_DEBUG=1` / `POST /api/debug {"enabled":true}` 开关，默认关闭；当前记录入口过滤、日志准备、协议转换和上游 `open()` 阶段，已有请求完成/失败总耗时日志继续保留。后续在不改变路由、重试和响应语义的前提下补齐请求读取、上游首包/总时长、响应转换/中继、payload 字节数、重试次数与端点切换次数，并用同一 payload、同一端点、同一时间窗口建立 Hermes 直连与 API Pool 对照基线。
- [x] **[P1] 将敏感词故障诊断移出正常请求热路径** — `_try_endpoint()` 不再每次尝试固定执行 3 次 `_cf_probe_count()` 和 2 次 `_cf_probe_paths()`；仅在 DEBUG 且出现对应拦截错误时，对当前失败尝试的 payload 即时诊断。已移除共享 `_last_cf_diag`，改为调用栈局部状态，避免正常请求/普通失败的重复全量遍历及并发请求互相覆盖诊断结果；新增定向测试覆盖默认关闭、正常路径零扫描和局部诊断。
- [ ] **[P1] 评估并改造完整对话日志写入管线** — 量化 `extract_prompt_text(payload)` 的遍历/字符串副本成本、每请求后台线程数量、SQLite 锁等待、写入时延、数据库增长和高并发内存峰值；在证据明确后再选择单写入队列/批量事务、日志截断或仅元数据等最小方案。**本阶段不直接取消完整 prompt/completion 日志，也不改变 30 天保留策略。**
- [ ] **[P1] 建立可重复的 payload 规模基准** — 至少覆盖 10/50/100/250/500 KB，分别测量过滤、诊断、日志准备、OpenAI 透传和 Anthropic 转换的 P50/P95/P99；测试使用本地 mock 上游，不向生产端点发送探测请求。
- [ ] **[P2] 基于分段数据复核重试与故障转移长尾** — 区分 API Pool 固定开销、上游等待、内部重试、候选探活和端点切换耗时；只在数据证明收益后调整超时/探活策略，不把小请求探活结果等同于长上下文真实请求能力。

**阶段边界**：先补观测和基准，再做最小热路径优化；不重构单文件部署形态，不引入新的路由状态机，不改变现有 Endpoint 选择、cooldown、defer、fallback、过滤覆盖和协议兼容语义。
