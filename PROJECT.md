...
| 2026-09-01 | 统计卡移动端 2 列修复 | 今日消耗等 8 个统计卡在手机显示 4 列 79px（数字挤压）。根因：`.dash-stats` 基础规则（`repeat(4,1fr)`）定义在移动端 media query **之后**，同特异性后定义胜 → L58/L130/L162 的 2 列/1 列覆盖**从未生效**。修复：删除失效死规则，在 `.dash-stats` 基础之后追加 ≤768px 2 列（gap 10px/padding 14px）与 ≤480px 2 列（gap 6px/padding 10px/数字 16px），380px 以下不再降 1 列（用户要求恒定 2 列）。375px 实测 8 卡 2 列 178px、320px 2 列 151px；PC 4 列不变。生产 hash `76ad146f`。⚠️ 同类 CSS 顺序问题仍影响 `.card/.stat-item` 等 L170+ 主样式组件（移动端 padding 覆盖同样未生效），待用户确认是否整体修复。 |
