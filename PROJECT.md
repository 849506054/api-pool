...
| 2026-09-01 | runtime_state 事件驱动落盘（方案 A） | 热路径零写盘：`_on_success` 自动路由成功只更新内存指针，不再实时读写 `api_runtime_state.json`（原每次端点切换 load+save+fsync 两个同步 IO）；手动切换/模型切换等显式操作即时写盘；`main()` 注册 SIGTERM handler 停止前全量快照（`snapshot_runtime_state()`，manual 优先，无指针组不写键）；启动恢复对「组名已不存在 / 端点 id 已不存在」的残留键自愈清理（组改名/删除、端点删除遗留，修复重启重复 WARN，生产 2026-09-01 清理 api-pool-bg/api-pool-gpt 残留）。崩溃场景由启动恢复 + 残留自愈兜底（文件为上次停止/手动状态）。6 项新增测试 + 全量 215 项通过；本地 mock E2E 验证热路径零写盘→手动切换写盘→SIGTERM 快照→重启恢复；生产 hash `6c4cc76a`，重启后 0 WARN，快照日志 `停止前运行态快照已保存（2 个组）`。旧方案残留 `_state_persistence_disabled` 已清理（写盘逻辑删除后无消费点）。commit `11445b1`。 |
| 2026-09-01 | 数据面板页移动端单列化 | 📊 数据面板两个 `1fr 1fr` grid（趋势/组成图表区、今日/本月排行区）在窄屏被压成 ~180px 双列卡片（图表 136px 宽），无移动端覆盖。新增 `.analytics-grid` class：PC 保持两列（与原 inline 完全等价），`@media(max-width:768px)` 单列堆叠 + gap 12px。390px 实测 4 卡全宽 378px 单列、图表 136→340px、无横向溢出；图表 SVG 按 `clientWidth` 重绘自适应。前端热更新直推生产（hash `45bb4e6b`），无需重启。 |
| 2026-09-01 | 统计卡移动端 2 列修复 + CSS 顺序整体修复 | 今日消耗等 8 个统计卡在手机显示 4 列 79px（数字挤压）。根因：`.dash-stats` 基础规则（`repeat(4,1fr)`）定义在移动端 media query **之后**，同特异性后定义胜 → L58/L130/L162 的 2 列/1 列覆盖**从未生效**（`.card/.stat-item` 等 L170+ 主样式组件同理，移动端 padding 覆盖同样失效）。修复分两步：①删除失效死规则，在 `.dash-stats` 基础之后追加 ≤768px 2 列（gap 10px/padding 14px）与 ≤480px 2 列（gap 6px/padding 10px/数字 16px），380px 以下不再降 1 列（用户要求恒定 2 列）；②把整个移动端 media 适配区（920px/768px×2/480px/380px 共 110 行）后移到主样式区之后，所有移动端覆盖恢复生效。375px 实测统计卡 2 列 178px、卡片 padding 10px 12px、图表单列 363px；1440px PC 零变化（4 列/两列/16px 18px）。生产 hash `195914f4`。 |
| 2026-09-01 | 趋势图表三视图尺寸不一致修复 | 「今日各时段消耗趋势」切到未命中/请求数时图表比 Token 矮。根因：`loadAnalytics` 的 setTimeout 内三个图表同时绘制，但 `missedTodayChart`/`callsTodayChart` 容器是 `display:none`（`clientWidth=0`）→ `drawSVGChart` 回退 `w=800` 生成 viewBox `800×180`，切回显示时 SVG 等比压缩变矮（363px 容器下高仅 ~82px）。修复：绘制前三个容器统一 `display:block`，绘制完恢复 none。实测三视图 viewBox 均为 `0 0 337 180`、显示 337×180 一致。生产 hash `03043963`。 |
...
- [x] **P1 Hermes 流式错误结束 E2E（2026-08-31 完成）** — 使用本地 mock 上游 + 本地 API Pool HTTP Handler + OpenAI SDK + Hermes `interruptible_streaming_api_call()` 真实链路验证：API Pool wire 输出 `finish_reason:error` 与可见总时长截断原因，截断后的上游内容不透传、不重复输出、不冻结端点；Hermes 按既有恢复语义将“部分输出后的流内 error”规范化为 `finish_reason:length` partial stub，完整保留已输出内容和截断原因并进入续写路径。新增 1 项 E2E，全量 190 项通过；不改生产代码，不部署、不重启。
- [x] **P1 重试边界收口（2026-08-31 已部署）** — `max_retries` 在新增/更新/对象构造三处入口统一归一化 0..3（非法值回退 1，生产 24 端点现值全为 1，行为零变化）；图片转译（同步与流式桥接两分支）、流式最终目标调用、temperature/top_p 清洗递归全部继承 request ID 与请求截止预算；上游调用 timeout 现按剩余预算收紧，预算耗尽直接跳过调用返回明确错误（此前 deadline 只拦退避重试、不拦单次 urlopen）。主轮转、流式停滞内部重试、bg→main fallback 原本已继承核对通过。5 项新增回归 + 全量 195 项通过；本地 mock E2E 验证 99→3、-7→0、更新 42→3 与配置落盘；生产 md5 `c1a1b637460ea1c6153f95a1d6171759`，重启后新世代 0 ERROR/0 WARN，真实流量请求成功。commit `26958e9`。
