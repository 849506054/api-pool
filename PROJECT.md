...
| 2026-09-01 | runtime_state 事件驱动落盘（方案 A） | 热路径零写盘：`_on_success` 自动路由成功只更新内存指针，不再实时读写 `api_runtime_state.json`（原每次端点切换 load+save+fsync 两个同步 IO）；手动切换/模型切换等显式操作即时写盘；`main()` 注册 SIGTERM handler 停止前全量快照（`snapshot_runtime_state()`，manual 优先，无指针组不写键）；启动恢复对「组名已不存在 / 端点 id 已不存在」的残留键自愈清理（组改名/删除、端点删除遗留，修复重启重复 WARN，生产 2026-09-01 清理 api-pool-bg/api-pool-gpt 残留）。崩溃场景由启动恢复 + 残留自愈兜底（文件为上次停止/手动状态）。6 项新增测试 + 全量 215 项通过；本地 mock E2E 验证热路径零写盘→手动切换写盘→SIGTERM 快照→重启恢复；生产 hash `95267bba`，重启后 0 WARN，快照日志 `停止前运行态快照已保存（2 个组）`。commit `f5e0f18`。 |
