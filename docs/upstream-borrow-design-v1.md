...
## §6 P2 整合评审：路由切换与恢复（2026-08-27 并入）

PROJECT.md 中四项 P2 待办实为同一决策空间的两面，应作为一个整体评审而非分散实施：

### 问题定义
sticky 端点的 prompt cache 按前缀命中。每次非必要切换 = 新端点全价重算 system+历史；
每次不必要的留守（卡死上游）= 请求高延迟甚至失败。目标是**最小化切换次数的同时保住故障逃逸能力**。

### 四项待办在决策链上的位置

| 待办 | 决策阶段 | 作用 |
|------|----------|------|
| 客户端类错误分类（本稿改动二） | 触发面收窄 | 把参数类 400 移出切换理由，消除假阳性切换 |
| 存量：连续 N 次失败才切换 | 触发面收窄 | 单次瞬态失败不切，原地重试（与 `_try_endpoint` 内部 retry 关系需厘清） |
| 存量：降权不冻结 / deferrable 现状 | 记账方式 | 失败后标记降权但保持可回落，替代硬冷却 |
| 存量：首包超时 120s→30~45s | 触发阈值 | 卡死上游的等待代价减半以上；仅调 first-packet，不动 stall/max_duration |
| 确定性抖动冷却（本稿改动一） | 恢复面 | 解冻错峰，避免整池同时回流打挂同一上游 |

### 评审输入依赖
P1「观察本轮三个修复的实际行为」产出的真实样本是本评审的前置数据——延迟回切、
探活误判的实际发生率决定 N 取值与是否需要降权通道。**在观察数据到手前不实施任何一项**，
避免用猜测参数替代实测。

### 设计边界（沿用既有约束）
- 不引入复杂路由状态机 / ModelRoute / 独立 Upstream 建模（存量硬边界）；
- 三项独立提交的实施顺序保持：原子写 → 抖动 → 分类；存量三方案在本框架下二次评估；
- 改动二的 A′ 方案已覆盖「连续 N 次失败」的场景子集（400 类不再产生失败计数），
  存量方案落地时须与之合并设计，避免双重记账口径。
